...
let handler = async (m, { conn, usedPrefix, text }) => {
  ...
  if (new Date - global.db.data.users[m.sender].lastmulung< 1800000) throw `Anda sudah lelah untuk mulung\nTunggu selama ${msToTime(time - new Date())} lagi`
	let botolnye = `${Math.floor(Math.random() * 999999999)}`.trim()
	let kalengnye = `${Math.floor(Math.random() * 999999999)}`.trim()
	let kardusnye = `${Math.floor(Math.random() * 999999999)}`.trim()
